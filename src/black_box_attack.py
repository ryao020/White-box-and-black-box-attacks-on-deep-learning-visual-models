...
def test_with_adversarial_attacks(steps=100, epsilon=0.1, args=None):
    ...
    for trail in range(args.trails_number):
        ...
        for step in range(steps):
            ...
            action = target_model(state_tensor).max(1)[1].view(1, 1).item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source_model_path", type=str, default="policy_net_dqn1.pth")
    parser.add_argument("--target_model_path", type=str, default="policy_net_dqn2.pth")
    ...
    print("sourece_model_path: ", args.source_model_path)
    print("target_model_path: ", args.target_model_path)

    # Load the trained model
    source_model = DQN(state_dim, action_dim).to(device)
    source_model.load_state_dict(torch.load(args.source_model_path))
    source_model.eval()
